...
class PointNavEnv:
    """
    Gym environment for doing the PointNav task on the Go1 robot with ZED camera in the real world.
    """
    ...
    def _get_nav_depth(self) -> np.ndarray:
        """Get depth image from ZED camera for navigation.
        
        For Go1 with ZED, we only have one camera (LEFT), so we use it directly.
        If depth map is available, use it; otherwise, convert RGB to grayscale as placeholder.
        """
        # Get depth map directly from ZED camera
        depth_img = self._get_zed_depth()
        
        # Normalize depth
        depth_img = self._norm_depth(depth_img)
        return depth_img
    ...
    def _norm_depth(self, depth: np.ndarray, max_depth: Optional[float] = None, scale: bool = False) -> np.ndarray:
        """Normalize depth image to [0, 1] range.
        
        Args:
            depth: Depth image in meters (H, W)
            max_depth: Maximum depth value for normalization
            scale: Not used for ZED (depth is already in meters)
        
        Returns:
            np.ndarray: Normalized depth image (H, W, 1) in [0, 1]
        """
        ...
        # ZED depth is already in meters, no need to convert from mm
        # Clip to max_depth and normalize to [0, 1]
        norm_depth = np.clip(norm_depth, 0.0, max_depth) / max_depth
        
        # Add channel dimension if needed (for compatibility with policy input)
        if len(norm_depth.shape) == 2:
            norm_depth = norm_depth[:, :, np.newaxis]
